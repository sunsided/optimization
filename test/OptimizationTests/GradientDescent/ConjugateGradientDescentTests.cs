...
using OptimizationTests.Hypotheses;
...
{
    public sealed class ConjugateGradientDescentTests
    {
        [Fact]
        public void LinearRegressionWithResidualSumOfSquares()
        {
            ...
            var problem =
                new OptimizationProblem<double, IDifferentiableCostFunction<double>>(costFunction, initialCoefficients);
        ...
        public void FletcherReevesRosenbrockParameterFitWithResidualSumOfSquares()
        {
            ...
            // define the optimization problem
            var problem =
                new OptimizationProblem<double, IDifferentiableCostFunction<double>>(costFunction, initialTheta);
        ...
        public void PolakRibiereUnivariateExponentialRegressionWithResidualSumOfSquares()
        {
            ...
            // assert!
            var coefficients = result.Coefficients;
            coefficients[1].Should()
                .BeApproximately(theta[1], 1000D, "because that's the underlying system's [a] parameter");
            coefficients[2].Should()
                .BeApproximately(theta[2], 1E-2D, "because that's the underlying system's [b] parameter");
        ...
        public void FletcherReevesRosenbrock()
        {
            ...
            // assert!
            var coefficients = result.Coefficients;
            coefficients[0].Should().BeApproximately(rosenbrockParameter[0], 1E-5D,
                "because the Rosenbrock function has a minimum at x={0}, y={1}", rosenbrockParameter[0],
                Math.Sqrt(rosenbrockParameter[0]));
            coefficients[1].Should().BeApproximately(Math.Sqrt(rosenbrockParameter[0]), 1E-5D,
                "because the Rosenbrock function has a minimum at x={0}, y={1}", rosenbrockParameter[0],
                Math.Sqrt(rosenbrockParameter[0]));
        }
