...
namespace WideMeadows.Optimization.GradientDescent.Conjugate
{
    ...
    /// </code>
    /// </remarks>
    public sealed class PolakRibiereCG : DoublePrecisionConjugateGradientDescentBase<IDifferentiableCostFunction<double>>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="FletcherReevesCG"/> class.
        /// </summary>
        /// <param name="lineSearch">The line search.</param>
        public PolakRibiereCG(ILineSearch<double, IDifferentiableCostFunction<double>> lineSearch)
            : base(lineSearch)
        {
        }
        ...
        /// <summary>
        /// Gets a pre-conditioner for the residuals.
        /// </summary>
        /// <param name="problem">The problem.</param>
        /// <param name="theta">The coefficients to optimize.</param>
        /// <returns>MathNet.Numerics.LinearAlgebra.Matrix&lt;System.Double&gt;.</returns>
        private Matrix<double> GetPreconditioner(IOptimizationProblem<double, IDifferentiableCostFunction<double>> problem, Vector<double> theta) =>
            // sadly we have no clue.
            Matrix<double>.Build.DiagonalIdentity(theta.Count);
