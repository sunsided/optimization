...
namespace WideMeadows.Optimization.GradientDescent.Regular
{
    ...
    public sealed class ResilientErrorGD : GradientDescentBase<double, IDifferentiableCostFunction<double>>
    {
        ...
        {
            get => _stepIncreaseFactor;
            set
            {
                if (double.IsNaN(value) || double.IsInfinity(value)) throw new NotFiniteNumberException("The value must be finite", value);
                if (value <= 1) throw new ArgumentOutOfRangeException(nameof(value), value, "The value must be greater than 1");
        ...
        {
            get => _stepDecreaseFactor;
            set
            {
                if (double.IsNaN(value) || double.IsInfinity(value)) throw new NotFiniteNumberException("The value must be finite", value);
                if (value is <= 0 or >= 1) throw new ArgumentOutOfRangeException(nameof(value), value, "The value must be in range ]0..1[");
